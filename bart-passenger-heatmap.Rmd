...
1. Parse a base input graph to set up the network topology, such as the graph on the right above. The graph is considered to be unweighted and undirected.
...
This significantly cuts down the final file size.

As most of the code was identical, I wrote the preparser in Python, again leveraging `NetworkX` for its graph functions.
...
Splitting the code into clearly separate pieces made sense to me.
...
I needed separate lines between each station, so that I could color each of them separately depending on how many passengers travelled on that edge.
...
You can find both of these files in the `geojson/` subdirectory.
...
Combining these together, I was able to successfully render the tracks.
...
* `fasttime` to quickly convert the dates in the preparsed data to `POSIXct`
...
These features built off the original base application that I wrote.
...
I wanted the tracks to be re-rendered if those values are changed, so I needed to make a reactive variable for the track data and number of passengers on each piece of track.
...
If it is, then it re-renders the track data using the function [`leafletProxy()`](https://www.rdocumentation.org/packages/leaflet/versions/2.0.2/topics/leafletProxy) in `leaflet`.
The track data is dependent on `passengerData`, so that will be updated as well.
It will re-subset the data based on the user input and update the variable.
...
Another useful side efect is that nothing is re-rendered, saving some processing time.
Since `passengerData` isn't used if the input is invalid, it isn't updated either, so we save time there as well.

Note that the Shiny application also requires other input files, like the GeoJSON data to render the stations and tracks, the prepared data CSV, and the icon used to indicate where a station is.
